...
const Gift = (props) => {
  ...
  useEffect(() => {
    ...
    if (selectedIndex.row == 0) {
      setSelectedToken({
        symbol: nativeToken,
        amount: Number(balanceData.NativToken),
        contractAddress: null,
      });
    } else {
      setSelectedToken({
        symbol: balanceData.ERC20Token[selectedIndex.row - 1].symbol,
        amount:
          Number(balanceData.ERC20Token[selectedIndex.row - 1].balance) /
          10 ** Number(balanceData.ERC20Token[selectedIndex.row - 1].decimals),
        contractAddress:
          balanceData.ERC20Token[selectedIndex.row - 1].token_address,
        decimals: balanceData.ERC20Token[selectedIndex.row - 1].decimals,
      });
    }
  }, [selectedIndex]);
  ...
  const sendout = () => {
    ...
    if (WalletInUse == 1) {
      ...
      transferToken(
        ...
        console.log("tw wallet transfer native token result: ", res);
      });
      ...
      >
        <View>
                ...
                >
                  ...
                  <Text>
                    {selectedToken.amount
                      ? Number(selectedToken.amount).toFixed(6)
                      : null}
                  </Text>
                </View>
              </View>
          ...
          >
            ...
            {balanceData.ERC20Token &&
              balanceData.ERC20Token.map((item) => (
                <SelectItem
                  title={
                    <View>
                      <View
